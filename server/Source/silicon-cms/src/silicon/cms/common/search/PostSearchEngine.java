package silicon.cms.common.search;
...
public class PostSearchEngine {
	...
	private PostSearchEngine()
	{
		...
		{
			throw new SCRuntimeException(e2);
		}
		try
		{
			_indexWriter.commit();
	...
	private Directory getIndexDirectory()
	{
		if (_indexDirectory == null)
		{
			try
			{
				_indexDirectory = FSDirectory.open(SCEnvironment.getSharedFile("indexes"));
			}
			catch (IOException e)
			{
				throw new SCRuntimeException(e);
			}
		}
		return _indexDirectory;
	}
	
	public void buildIndex(GoodsEntity p_po)
	{
	...
	public void updateIndex(GoodsEntity p_vo)
	{
